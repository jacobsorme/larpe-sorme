...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size, lamb, learn, buffer, freq, units, layers): # lamb=0.95, lr=0.005, mem_sz=1000, freq=1, layers=1, units=16):
       ...
       # Modify here
        ...
        self.model = self.build_model(layers, units)
        self.target_model = self.build_model(layers, units)
...
###############################################################################
    ...
    def build_model(self, layers, units_per_layer):
        model = Sequential()
        model.add(Dense(units_per_layer, input_dim=self.state_size, activation='relu', kernel_initializer='he_uniform'))

        for l in range(layers-1):
            model.add(Dense(units_per_layer, activation='relu', kernel_initializer='he_uniform'))
...
###############################################################################
        ...
        for i in range(self.batch_size): #For every batch
            if done[i]:
                ...
                y = reward[i] + self.lamb * np.max(target_val[i])
            target[i][action[i]] = y
###############################################################################
###############################################################################
        ...
        self.model.fit(update_input, target, batch_size=self.batch_size,
                       epochs=1, verbose=0, use_multiprocessing=True)
        return
